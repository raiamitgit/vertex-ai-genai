-- generate_synthetic_data_30k.sql
-- BigQuery ML Generator for Data Gravity Demo (PRODUCTION SCALE: 30,000 ROWS)
-- Batched execution, chunking API requests via 5,000 rows per loop

-- 1. Setup DDL: Create Connection and Model
...
CREATE OR REPLACE TABLE `tuning.cpt_data_30k` (
  id STRING,
  category STRING,
  theme STRING,
...
CREATE OR REPLACE TABLE `tuning.sft_data_30k` (
  cpt_id STRING,
  messages JSON
);

CREATE OR REPLACE TABLE `tuning.lora_data_30k` (
  cpt_id STRING,
  messages JSON
);

-- TRUNCATE TABLE `tuning.cpt_data_30k`;
...
INSERT INTO `tuning.cpt_data_30k` (id, category)
SELECT
  GENERATE_UUID() AS id,
...
WHERE NOT EXISTS (SELECT 1 FROM `tuning.cpt_data_30k` LIMIT 1);

-- 0.2: Generate Themes and Descriptions via Gemini (Batched)
-- Batch size: 5000
WHILE EXISTS (SELECT 1 FROM `tuning.cpt_data_30k` WHERE theme IS NULL) DO
  ...
  UPDATE `tuning.cpt_data_30k` AS t
  SET 
...
-- Updates the `cpt_data_30k` master table dynamically based on category. (Batched)
-- ==============================================================================
WHILE EXISTS (SELECT 1 FROM `tuning.cpt_data_30k` WHERE raw_artifact IS NULL AND theme IS NOT NULL) DO
...
-- Updates the `cpt_data_30k` master table with structured QA based on the artifacts. (Batched)
-- ==============================================================================
WHILE EXISTS (SELECT 1 FROM `tuning.cpt_data_30k` WHERE q_and_a_pairs IS NULL AND raw_artifact IS NOT NULL) DO
...
-- ==============================================================================
TRUNCATE TABLE `tuning.sft_data_30k`;

INSERT INTO `tuning.sft_data_30k` (cpt_id, messages)
SELECT
  id AS cpt_id,
  JSON_ARRAY(
...
-- ==============================================================================
TRUNCATE TABLE `tuning.lora_data_30k`;

INSERT INTO `tuning.lora_data_30k` (cpt_id, messages)
SELECT
  id AS cpt_id,
  JSON_ARRAY(
      JSON_OBJECT('role', 'system', 'content', 'You are an expert AI security analyst assistant.'),
